    ...
    try {
      ...
      if (activeTab === 'written') {
        ...
        response = await axios.get(
          `/replies/seller/${user._id}?full_name=true`
        );
      }
      ...
      if (response.data.ok) {
        if (activeTab === 'received') {
          ...
          const allReplies = response.data.item.flatMap(
            (product: {
              _id: number;
              product_id: number;
              name: string;
          ...
          <div>
            {reviews.map((item, index) => (
              <div key={item._id}>
                ...
                >
                  {/* 프로필 이미지 또는 이니셜 */}
                  <div className="w-18 h-18 shrink-0 relative pointer-events-none">
                    {activeTab === 'received' ? (
                      // 받은 후기: 작성자 정보
                      item.user?.image ? (
                        <Image
                      ...
                      // 작성한 후기: 상품 이미지
                      <Image
                        src={getImageUrl(item.product?.image?.path)}
